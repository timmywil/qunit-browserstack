...
QUnit.test("login test", async (assert) => {
  ...
  const text = await driver.findElement(By.css(".api-error")).getText();
  assert.equal(text, "Your account has been locked.");
}, 1000000);
...
QUnit.test(
  ...
  async (assert) => {
    ...
    assert.equal(productText, productCartText);
  },
  10000000
);
